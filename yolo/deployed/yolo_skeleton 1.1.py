#yolo26 headless daily file rotation auto reconnect
import cv2
import time
import sys
import os
...
class HeadlessTracker:
    ...
    def init_logs(self):
        ...
        if pose_needs_header:
            header = ["Timestamp", "ID", "BBox_X1", "BBox_Y1", "BBox_X2", "BBox_Y2"] + [f"KP{i}_{c}" for i in range(17) for c in ["X","Y","C"]]
    ...
    def run(self):
        ...
        while True:
            ...
            if time.time() - self.last_print_time >= 10.0:
                ...
                print(f"[{capture_ts}] Heartbeat | FPS: {fps:.1f} | People Visible: {person_count}")
                self.frame_count = 0
                self.last_print_time = time.time()
